...
During the final four weeks of 2015 p-p collisions, data-taking proceeded smoothly with a loss of less than 1% of the collected data due to HCAL problems *10.7/pb  out of 1,184/pb). This good performance continued in the heavy-ion running.
...
![DAQ File Based Filter Farm Monitor page showing the merger status for each Lumi Section for one of the last runs of Heavy Ion running period.  Total output from all HLT nodes was ~5.3 GB/s. The figure also lists the large number of different output streams HLT nodes produced.][image-2]
...
[image-2]:	figures/PastedGraphic-1.pdf {#fig:daq}